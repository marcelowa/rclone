...
import (
	...
	"github.com/rclone/rclone/backend/telegram/telegramfile"
...
// Options represent the configuration of the KV backend
type Options struct {
	AppId     int
	AppHash   string
	ChannelId int64
...
func (f *Fs) getDb() (*kv.DB, error) {
	var err error
	if f.db == nil {
		f.db, err = kv.Start(context.Background(), f.name, f)
		if err != nil {
			return nil, err
		}
	}
...
func (o *Object) getFromTelegram() (io.ReadCloser, error) {
	channelID := o.fs.opt.ChannelId
	messageID := int(56)
	// messageID := 35
	config := telegramfile.Config{
		AppID:     o.fs.opt.AppId,
		AppHash:   o.fs.opt.AppHash,
		BotTokens: strings.Split(o.fs.opt.BotTokens, " "),
	}
	client, _, err := telegramfile.AuthenticatedClient(context.Background(), config)
	if err != nil {
		return nil, err
	}

	return telegramfile.GetFileReaderFromMessage(context.Background(), client, channelID, messageID)
}
